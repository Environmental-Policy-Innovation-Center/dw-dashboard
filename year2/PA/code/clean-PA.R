clean_pa_y2 <- function() {
  ...
  pa_fundable <- fread("year2/PA/data/pa-fundable-ppl.csv",
    ...
    clean_names() %>%
    # [keep] this removes 3 projects that are the PENNVEST / pending projects 
    filter(!is.na(epic_merge_id)) %>%
    # [keep]  all of these are expecting funding 
    mutate(expecting_funding = "Yes", 
           funding_amount = clean_numeric_string(total_assistance_amount)) 
  
  # [keep]  comp list: note there was some light tidying of borrower column manually 
  # [keep]  (i.e., adding "-" between borrower and project)
  pa_comp <- fread("year2/PA/data/pa-base-ppl.csv",
                   colClasses = "character", na.strings = "") %>%
    clean_names() %>%
    # [keep] community_served corresponds to geographic reference in the dd
