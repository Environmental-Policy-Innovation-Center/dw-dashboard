...
me_raw <- fread(paste0(ppl_folder_path, "19-Maine_PPL_Comprehensive.csv"),
                colClasses = "character", na.strings = "") %>%
  clean_names()

me_clean <- me_raw %>%
  # drop columns
  select(-pf_percent, -arpa_mjrp, -percent_annual_water_bill_of_mhi, -project_type) %>%
  # process numeric columns
  mutate(funding_amount = as.numeric(str_replace_all(amount_requested, "[^0-9.]", "")),
         ...
         population = as.numeric(str_replace_all(population_served, "[^0-9.]", "")),
         # TODO: need to define BP
         state_score = as.numeric(str_replace_all(total_priority_points, "[^0-9.]", ""))) %>%
  # process text columns
  ...
  select(borrower, pwsid, state_score, project_name, funding_amount, 
         principal_forgiveness_amount, project_type, population, state, fundable)
```
  ...
  mutate(disadvantaged = case_when(
    ...
    TRUE ~ "Yes"
         ...
         fundable = case_when(
           ...
           grepl("Yes", estimated_loan_amount) ~ "Yes",
           TRUE ~ "No")
  ) %>%
  filter(fundable == "Yes") %>%
