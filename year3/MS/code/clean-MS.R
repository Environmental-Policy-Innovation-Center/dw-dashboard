clean_ms_y3 <- function() {

  curated_file_names <- stringr::str_subset(list.files("year3/MS/data"), "curated")
  ...
  for (file_name in PPL_files){
    tmp_priority_list <- data.table::fread(paste0("year3/MS/data/", file_name), colClasses = "character", na.strings = "") |> 
      janitor::clean_names() |>
      dplyr::mutate(
  ...
  for (file_name in PPlaL_files){
    tmp_planning_list <- data.table::fread(paste0("year3/MS/data/", file_name), colClasses = "character", na.strings = "") |> 
      janitor::clean_names() |>
      dplyr::mutate(
...
combined_list <- priority_list |> 
  ...
  ms_clean <- combined_list |>
    ...
    dplyr::mutate(
      ...
      list = paste0("SFY25 ", source_file, " ", source_list, " List")
    ) |>
