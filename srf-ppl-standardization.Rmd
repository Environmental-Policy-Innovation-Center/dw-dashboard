...
ga_clean <- ga_raw %>%
  ...
  select(state_score, borrower, project_description, funding_amount, principal_forgiveness_amount,
         population, fundable, project_type, state)
...
mn_clean <- mn_raw %>%
  ...
  mutate(
    ...
    est_wif_grant = as.numeric(str_replace_all(estimated_wif_grant_not_final_4,"[^0-9.]","")),
    est_wif_grant = replace_na(est_wif_grant, 0),
    # Sum of Estmated DWRF Lead, Estimated DWRF Emerging Contaminant, 
    # Estimated DWRF Disadvantaged Community, Estimated WIF Grant
    principal_forgiveness_amount = est_lslr_pf + est_ec_pf + est_dac_pf + est_wif_grant,
