...
project_counts <- project_counts |>
  dplyr::left_join(lead_type_count_summary |> dplyr::mutate(state_fiscal_year = as.numeric(state_fiscal_year)))
```
...
fa_by_lsl <- lsl_projects |>
  ...
  dplyr::mutate(
    funding_amount_lslb = if("both" %in% dplyr::cur_data()) both else NA_real_,
    funding_amount_lslr = if("lslr" %in% dplyr::cur_data()) lslr else NA_real_,
    funding_amount_lsli = if("lsli" %in% dplyr::cur_data()) lsli else NA_real_,
    funding_amount_lslu = if("unknown" %in% dplyr::cur_data()) unknown else NA_real_
  ) |>
  dplyr::select(state, state_fiscal_year, funding_amount_lslb, funding_amount_lslr, funding_amount_lsli, funding_amount_lslu)
...
fa_by_lsl_ef <-  lsl_projects |>
  ...
  dplyr::select(state, state_fiscal_year, funding_amount_ef_lslb, funding_amount_ef_lslr, funding_amount_ef_lsli, funding_amount_ef_lslu)
