clean_pa_y1 <- function() {
  ...
  missing_fundable_project <- pa_fundable %>%
    ...
    select(-c(epic_merge_id, disadvantage_ej_area))
  ...
  pa_clean <- pa_clean |>
    dplyr::mutate(
      project_type = dplyr::case_when(
        project_id == "82233" ~ "Emerging Contaminants", #project_name == "Water Treatment Plant & System Upgrades Project" 
        project_id == "82230"	~	"Both Lead and EC", #project_name == "(Swcma) Upgrade"
