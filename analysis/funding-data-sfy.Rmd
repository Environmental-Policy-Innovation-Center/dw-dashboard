...
```{r}
# categorize lead projects as Inventory or Replacement
lead_projects <- state_projects %>%
  filter(project_type == "Lead") %>%
  mutate(lsl_category = case_when(
    grepl("Inventory", project_description, TRUE) & grepl("Replacement", project_description, TRUE) ~ "Both",
    grepl("Inventory", project_description, TRUE) ~ "Inventory",
    grepl("Replacement", project_description, TRUE) ~ "Replacement",
    TRUE ~ "Neither")
  ) %>%
  group_by(lsl_category) %>%
  summarize(count=n(),
            value = sum(as.numeric(project_cost), na.rm=TRUE)
            ) %>%
  rename(name = lsl_category)

tibble(lead_projects)
```
