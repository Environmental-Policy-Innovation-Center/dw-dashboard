clean_wi_y4 <- function() {
  ...
  wi_lsl_comp <- fread("year4/WI/data/wi-sfy26-lsl-comp-list.csv",
                       colClasses = "character", na.strings = "") |>
    janitor::clean_names() |>
    dplyr::mutate(list = "SFY26 LSL Comprehensive List")
  ...
  wi_gen_ec_fundable <- fread("year4/WI/data/wi-sfy26-general-fundable-list.csv",
                              colClasses = "character", na.strings = "") |>
    janitor::clean_names() |>
    dplyr::mutate(list = "SFY26 General & EC Fundable List",
  ...
  wi_lsl_fundable <- fread("year4/WI/data/wi-sfy26-lsl-fundable-list.csv",
                           colClasses = "character", na.strings = "") |>
    janitor::clean_names() |>
    dplyr::mutate(list = "SFY25 LSL Fundable List",
           project_type = "Lead",
           community_served = stringr::str_to_title(municipality),
           project_id = stringr::str_squish(project_number),
           project_id = normalize_dashes(project_id),
           ...
           disadvantaged = ifelse(private_side_pf_percent != "0%", "Yes", "No"),
           project_score = stringr::str_squish(lsl_perf_score),
           expecting_funding = "Yes"
    ) |>
    dplyr::select(project_id, community_served, project_score, project_type, requested_amount, 
...
# 4922-21 is for the same municipality and almost identical project descriptions, but different scores and estimated project costs. Leaving both for now, but should consider combining?
