...
- Applicant data includes `applied for project amount` which can be interpretted as requested amount
...
# (364, 11)
al_applicant <- fread("data/srf-ppl/year1/1-Alabama_ARPA_SRF_Combined_PPL.csv",
...
# (7,14)
al_base <- fread("data/srf-ppl/year1/1-Alabama_Base-PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

# (19,14)
al_bil <- fread("data/srf-ppl/year1/1-Alabama_BIL-PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

# (3,14)
al_lead <- fread("data/srf-ppl/year1/1-Alabama_Lead-PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
al_clean <- al_comb %>%
  ...
  select(state_score, borrower, pwsid, project_name, project_description, funding_amount, principal_forgiveness_amount, requested_amount,
...
# (15,21)
ar_merge <- fread("data/srf-ppl/year1/4-Arkansas_Merged.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

# the comprehensive PPL includes applicant projects, (747,13)
ar_comp <- fread("data/srf-ppl/year1/4-Arkansas.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
ca_fund <- fread("data/srf-ppl/year1/5-California_Fundable_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
ca_ec <- fread("data/srf-ppl/year1/5-California_EC.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  # process numeric columns
  mutate(population = as.numeric(str_replace_all(population,"[^0-9.]", "")),
...
ca_app <- fread("data/srf-ppl/year1/5-California_Comprehensive_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
- TODO Applicant data, waiting for question resolution to dive in further
- Mentions *Interest Rates or Loan Terms*

```{r}

# (165,11)
co_raw <- fread("data/srf-ppl/year1/7-Colorado_AppendixB.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
- TODO: Need to scrape additional data for Applicants. Scrape is a WIP due to formatting issues.
- Mentions *Interest Rates or Loan Terms*

```{r}
de_fund <- fread("data/srf-ppl/year1/8-Delaware_PPL_fund.csv",
                      colClasses = "character", na.strings = "")
...
## District of Columbia (DD)

- Assumes `requested_amount` should be included, needs to be removed otherwise

```{r}

# 13,3
dc_ppl <- fread("data/srf-ppl/year1/DistrictofColumbia_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
dc_clean <- dc_ppl %>%
  ...
  requested_amount = as.numeric(str_replace_all(grant_funding,"[^0-9.]","")),
  state = "District of Columbia",
  project_type = "Other",
  fundable = "Applicant"
  ) %>%
  select(project_name, requested_amount, project_type, state, fundable)
...
- TODO: Applicant data - waiting for Q to be answered before addressing
- No mention of *Interest Rates or Loan Terms*

```{r}

fl_fund <- fread("data/srf-ppl/year1/9-Florida_ppl_fundable.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
- Applicant data is preprocessed and ready to be merged in at next update.
- Need to check for `requested_amount` columns, as Georgia seems to instead provide an estimated project cost, which could also be valuable.
- Mentions *Interest Rates or Loan Terms*
...
## BASE IUP
# (60,14)
ga_raw <- fread("data/srf-ppl/year1/10-Georgia_Comp_clean.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
ga_supp_1 <- fread("data/srf-ppl/year1/10-Georgia_Supplemental_IUP_A1.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

ga_supp_2 <- fread("data/srf-ppl/year1/10-Georgia_Supplemental_IUP_A2.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
## LEAD IUP
ga_lead_1 <- fread("data/srf-ppl/year1/10-Georgia_Lead_IUP_A1.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

ga_lead_2 <- fread("data/srf-ppl/year1/10-Georgia_Lead_IUP_A2.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  select(project, total_disburs) %>%
  # manually standardize names for matching
...
## EC IUP
# (2,10)
ga_ec_1 <- fread("data/srf-ppl/year1/10-Georgia_EC_IUP_A1.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

# (2,10) -> (2,2)
ga_ec_2 <- fread("data/srf-ppl/year1/10-Georgia_EC_IUP_A2.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  select(project, total_disburs) %>%
...
- Does not include a `requested_amount` or `project_cost`, so no new variables to include
- No mention of *Interest Rates or Loan Terms*

```{r}
# (33,8)
hi_fund <- fread("data/srf-ppl/year1/11-Hawaii_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
# (81, 9)
id_ppl <- fread("data/srf-ppl/year1/12-Idaho_PPL.csv", 
                      colClasses = "character", na.strings = "") %>% 
          clean_names()
...
id_fnd <- fread("data/srf-ppl/year1/12-Idaho_Fundable.csv",
                      colClasses = "character", na.strings = "") %>%
        clean_names() %>%
...
# (70,11)
il_ppl_f <- fread("data/srf-ppl/year1/13-Illinois_PPL_Fundable.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
# (19,11)
il_ppl_a <- fread("data/srf-ppl/year1/13-Illinois_PPL_Applicant.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
# (18,10)
il_lead_f <- fread("data/srf-ppl/year1/13-Illinois_Lead_Fundable.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(fundable="Fundable",
         project_type="Lead")
...
# (1,10)
il_lead_a <- fread("data/srf-ppl/year1/13-Illinois_Lead_Applicant.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(fundable="Applicant",
         project_type="Lead")
...
- IUP only includes Q1 2023, need to determine how to handle this in dashboard
- Can fundable range depict "fundable" or are they all applicant projects anyways?
...
## PPL
# (60,18)
in_ppl <- fread("data/srf-ppl/year1/14-Indiana_IUP_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()

## Lead
# (8,17)
in_lead_q123 <- fread("data/srf-ppl/year1/14-Indiana_PPL_Lead(Q1 2023).csv",
                 colClasses = "character", na.strings = "") %>%
  clean_names()

# -> (68,18)
in_comb <- bind_rows(in_ppl, in_lead_q123)
...
in_clean <- in_comb %>%  
  ...
  mutate(
    ...
    project_type = case_when(
      ...
      grepl("Lead service", project_description, ignore.case=TRUE) ~ "Lead",
      TRUE ~ "Other"
    ),
    state = "Indiana",
    fundable = "Applicant"
...
## Base
# -> (98,9)
ks_base <- fread("data/srf-ppl/year1/16-Kansas_PPL-base.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
## Lead
# -> (16,8)
ks_lead <- fread("data/srf-ppl/year1/16-Kansas_PPL-lead.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
# Emerging Contaminants
# -> (9,7)
ks_ec <- fread("data/srf-ppl/year1/16-Kansas_PPL-ec.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
```{r}
# (40,9)
la_ppl <- fread("data/srf-ppl/year1/18-Lousiana_Comprehensive_PPL-clean.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(fundable="Applicant")

# (8,11)
la_fnd <- fread("data/srf-ppl/year1/18-Lousiana_Fundable_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(fundable="Fundable")
...
me_ec <- fread("data/srf-ppl/year1/19-Maine_PL_EC.csv",
                colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
me_comp <- fread("data/srf-ppl/year1/19-Maine_PPL_Comprehensive.csv",
                colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  # drop columns
...
# (24,19)
md_raw <- fread("data/srf-ppl/year1/20-Maryland_PPL_manual.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
```{r}
# (120,7)
ma_comp_raw <- fread("data/srf-ppl/year1/21-Massachusetts_PPL.csv",
                      colClasses = "character", na.strings = "")
# (74,6)
ma_fund_raw <- fread("data/srf-ppl/year1/21-Massachusetts_Fundable.csv",
                      colClasses = "character", na.strings = "")
...
```{r}
# (112,24)
mi_raw <- fread("data/srf-ppl/year1/22-Michigan_PPL.csv",
                      colClasses = "character", na.strings = "")
# -> (110,12)
mi_clean <- mi_raw %>%
  ## clean names
...
```{r}
# (142, 16)
mn_raw <- fread("data/srf-ppl/year1/23-Minnesota_IUP_manual.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
# (48,9)
ms_raw <- fread("data/srf-ppl/year1/24-Mississippi_PPL.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
```{r}
# (43,12)
mo_ppl <- fread("data/srf-ppl/year1/25-Missouri_PPL_Cleaned.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
- TODO: Applicant data needs to be scraped from Appendix 2 of this doc - https://deq.mt.gov/files/Water/TFAB/DWSRF/IUP-PPL/2023_DWSRF_%20IUP_FINAL.pdf
- No mention of `requested_amount` or `project_cost`
- No mention of *Interest Rates or Loan Terms*

```{r}
# (37,4)
mt_raw <- fread("data/srf-ppl/year1/26-Montana_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() 
...
ne_base <- fread("data/srf-ppl/year1/27-Nebraska_ppl_base.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(project_type="Other")

# read in bil, all non-lead or ec projects, (13,9)
ne_bil <- fread("data/srf-ppl/year1/27-Nebraska_ppl_bil.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(project_type="Other")

# read in ec projects, (7,9)
ne_ec <- fread("data/srf-ppl/year1/27-Nebraska_ec.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(project_type="Emerging Contaminants")

# read in lead projects (19,9)
ne_lsl <- fread("data/srf-ppl/year1/27-Nebraska_lslr.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
```{r}
# (190,32)
nh_raw <- fread("data/srf-ppl/year1/29-NewHampshire_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
# (676, 19)
nj_raw <- fread("data/srf-ppl/year1/30-NewJersey_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
```{r}
# (6,13)
nm_raw <- fread("data/srf-ppl/year1/31-NewMexico_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
```{r}
# (172, 16)
nv_raw <- fread("data/srf-ppl/year1/28-Nevada_PPL.csv",
...
## Base
# (558,10) -> (558,12)
ny_base <- fread("data/srf-ppl/year1/32-NewYork_base_ppl.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  mutate(project_type = "Other",
         disadvantaged = case_when(
...
## BIL
# (120,10) -> (120,11)
ny_bil <- fread("data/srf-ppl/year1/32-NewYork_bil_ppl.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  select(-v1) %>%
  mutate(project_type = "Other",
...
## Lead
# (109,11) -> (109,12)
ny_lead <- fread("data/srf-ppl/year1/32-NewYork_lead.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  select(-v1) %>%
  mutate(project_type = "Lead",
...
## Emerging Contaminants
# (37,10) -> (37,11)
ny_ec <- fread("data/srf-ppl/year1/32-NewYork_ec.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  select(-pfas) %>%
  mutate(project_type = "Emerging Contaminants",
...
# (221, 15)
nc_raw <- fread("data/srf-ppl/year1/33-NorthCarolina_PPL_Comprehensive.csv",
                colClass="character", na.strings="") %>%
  clean_names()
...
## TODO: Replace these underlying files with the correct year, but maintain this code

nd_gen <- fread("data/srf-ppl/year1/34-NorthDakota_GeneralSupplementalFunding.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  mutate(project_type = "Other")


nd_ec <- fread("data/srf-ppl/year1/34-NorthDakota_EmergingContaminantsFunding.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  mutate(project_type = "Emerging Contaminants")


nd_lead <- fread("data/srf-ppl/year1/34-NorthDakota_LeadFunding.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
  mutate(project_type = "Lead")
...
# (262,14) -> (262,9)
nd_ppl <- fread("data/srf-ppl/year1/34-NorthDakota_PPL.csv",
                colClass="character", na.strings="") %>%
  clean_names() %>%
...
nd_clean <- nd_ppl %>%
  ...
  select(borrower, project_name, pwsid, project_type, project_description,
         state_rank, funding_amount, principal_forgiveness, disadvantaged, population, fundable, state)
...
## Base
# -> (440,9)
oh_base <- fread("data/srf-ppl/year1/35-Ohio_PPL_Base.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  select(-v1) %>%
...
## HAB Regional PF
#
oh_pf <- fread("data/srf-ppl/year1/35-Ohio_PPL_RegionalPF.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
## Lead
#
oh_lead <- fread("data/srf-ppl/year1/35-Ohio_LSLR.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
## Emerging Contaminants
# 
oh_ec <- fread("data/srf-ppl/year1/35-Ohio_PPL_HAB_PFAS.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  select(entity, project, estimated_loan_amount) %>%
  mutate(project_type = "Emerging Contaminants")
...
- Applicant data processed
- Similar to OH, how to handle `loan_amount` for projects we don't expect to be fundable? Currently assumes `requested_amount`
- No mention of *Interest Rates or Loan Terms*
...
## base / bil
# (69,12)
ok_b <- fread("data/srf-ppl/year1/36-Oklahoma_PPL-AppB.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(project_type = "Other") %>%
...
## lead
# (42,11)
ok_e <- fread("data/srf-ppl/year1/36-Oklahoma_PPL-AppE.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(project_type = "Lead") %>%
...
## emerging contaminants
# (42,11)
ok_f <- fread("data/srf-ppl/year1/36-Oklahoma_PPL-AppF.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  mutate(project_type = "Emerging Contaminants") %>%
  rename(disadvantaged = disadvantaged_y_or_n)
...
ok_clean <- ok_comb %>%
  ...
  select(borrower, state_score, project_name, project_description, requested_amount, disadvantaged, 
         population, project_type, state, fundable)
...
or_ppl_1 <- fread("data/srf-ppl/year1/Oregon_2023-1Q2023/2023 COMBINED (H&C&C) PPL-Table 1.csv",
                  colClasses = "character", na.strings = "") %>%
  clean_names()
...
```{r}
# (62,23)
pa_raw <- fread("data/srf-ppl/year1/38-Pennsylvania_manual_fundable_PPL.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
ri_comp <- fread("data/srf-ppl/year1/39-RhodeIsland_PPL_Comprehensive.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names() %>%
  # drop columns
...
ri_fund <- fread("data/srf-ppl/year1/39-RhodeIsland_PPL_Fundable_Manual.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names() %>%
...
# (20,11)
sc_raw <- fread("data/srf-ppl/year1/37-SouthCarolina_PPL.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
# (23,8)
sd_fnd <- fread("data/srf-ppl/year1/41-SouthDakota_Fundable.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()

# (136,8)
sd_ppl <- fread("data/srf-ppl/year1/41-SouthDakota_PPL2023.csv",
...
# (267,14) -> (265,9)
# 
tx_ppl <- fread("data/srf-ppl/year1/43-Texas_PPL.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names() %>%
...
tx_k <- fread("data/srf-ppl/year1/43-Texas_Fundable_K.csv",
               colClasses= "character", na.strings="") %>% 
  clean_names() %>%
...
ut_ppl <- fread("data/srf-ppl/year1/44-Utah_Comprehensive.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
# (99,12)
vt_ppl <- fread("data/srf-ppl/year1/45-Vermont_PPL.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
# (3,7)
vt_lead_t1 <- fread("data/srf-ppl/year1/45-Vermont_Lead_Table1.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()

# (70,7)
vt_lead_t2 <- fread("data/srf-ppl/year1/45-Vermont_Lead_Table2.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names() %>%
...
# Emerging Contaminants
# (17,7) -> (8,7)
vt_ec <- fread("data/srf-ppl/year1/45-Vermont_EC.csv",
                      colClasses = "character", na.strings = "") %>% 
  clean_names()
...
wa_merged <- fread("data/srf-ppl/year1/46-Washington_PPL_Merged.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
wv_raw <- fread("data/srf-ppl/year1/48-West_Virginia_Merged_Updated.csv",
                      colClasses = "character", na.strings = "") %>%
  clean_names()
...
```{r}
# (54,17) -> (46,19)
wi_fund_raw <- fread("data/srf-ppl/year1/49-Wisconsin_PPL.csv",
                     colClasses = "character", na.strings = "") %>%
    clean_names() %>%
  ## remove non project rows
...
# (4,17) -> (4,19)
wi_ec_fund <- fread("data/srf-ppl/year1/49-Wisconsin_EC_Funding_List.csv",
                    colClasses = "character", na.strings = "") %>%
  clean_names() %>%
...
wi_clean <- wi_fund_raw %>%
  ...
  mutate(## sum of columns, getting rid of $ and comma and replacing blanks with zero
         ...
         principal_forgiveness_amount = replace_na(principal_forgiveness_amount, 0),
         population = as.numeric(str_replace_all(population,"[^0-9.]", "")),
         requested_amount = as.numeric(str_replace_all(requested_,"[^0-9.]", "")),
         ) %>%
  ## non-numeric columns
...
# use this as a simple on/off switch for sending test data to AWS,
adding_test_data <- FALSE

if (adding_test_data == TRUE) { 

# create sample data for Gabe to test 1.1
# this data should include
# - states with funded AND applicant data (multiple)
# - states with only applicant data (DC)
# - states with new columns, project_cost (NJ) and requested_amount (OK)
combined_test <- combined_clean %>%
  # TODO: standardize all states so rank is CHAR, it was originally NUM but there are some states that use non-numeric symbols for ranking implying
  # projects are being bypassed or ranked based soley on DAC status or some other feature.
  mutate(state_rank = as.character(state_rank)) %>%
  bind_rows(dc_clean, nj_clean, ok_clean)

## Cleaning for table on web, currently leaves in Applicant projects for 1.1 release.
web_combined_clean_test <- combined_test %>%
...
data <- web_combined_clean_test
filename <- "web_ppl_combined_clean_v4.csv"
aws_filepath <- "clean_data/srf_project_priority_lists/web_ppl_combined_clean_v4.csv"
...
# remove Applicant data until Dashboard 1.1 ready
