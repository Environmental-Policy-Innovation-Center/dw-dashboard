...
sc_project_type_summary <- all_projects |>
  dplyr::filter(
    state == state_name
  ) |>
  dplyr::mutate(
  ...
  dplyr::mutate(
    ...
    state_fiscal_year = forcats::as_factor(state_fiscal_year)
  )

# Loop over project_type for plot per level
for (type_cat in levels(sc_project_type_summary$project_type)) {

  cat("Processing ", type_cat, "...\n")

  plot_title <- paste0("How many ", type_cat, " projects on the comprehensive list serve small communities?")
  

  # filter data frame per project type
  type_cat_df <- sc_project_type_summary |> dplyr::filter(project_type == type_cat) 
  ...
  sc_p <- type_cat_df |>
    ...
    labs(
      ...
      title = plot_title,
      subtitle = get_subtitle_str(sc_project_type_summary$state_fiscal_year, state_name)
    ) +
...
sc_gp <- ggplotly(sc_p, tooltip="text") %>%
    layout(title = list(text = paste0(plot_title,
                                      '<br>',
                                      get_subtitle_str(sc_project_type_summary$state_fiscal_year, state_name)
                                      )))
  ...
  cat("Saving ", type_cat, "plot ...\n")  
    run_save_plots(
      gg_plot = sc_p,
      gp_object = sc_gp, 
      name = paste0("serving-small-communities-", stringr::str_replace_all(stringr::str_to_lower(type_cat), " ", "-"), "-yoy") 
    )
  }
