...
funding_source_breakdown <- financial |>
  ...
  dplyr::mutate(
    source = dplyr::case_when(
      source == "total_srf_funds" ~ "DWSRF Funds",
      .default = "Other Federal and State Funds"),
    source = forcats::as_factor(source),
    source = forcats::fct_relevel(source, "Other Federal and State Funds","DWSRF Funds"),  
 ...
 other_sources <- funding_source_breakdown |>
  ...
  dplyr::mutate(
    ...
    source = "Other Federal and State Funds",
    plot_str = paste0("Other Federal and State Funds: ", format_currency(value) )
  )    
...
source_p <- funding_source_breakdown |>
  ...
  ggplot(aes(x=state_fiscal_year, y=value, fill=fed_cap_grant, alpha=source, text=plot_str)) + 
    geom_bar(stat="identity", position="stack") + 
    scale_alpha_manual(values = c("Other Federal and State Funds" = 0.5, "DWSRF Funds" = 1), name = "") +
    scale_y_continuous(labels=label_currency()) + 
    labs(x="", y="", fill="", alpha="Source",
         title="Total funding available from DWSRF and other federal and state funds",
...
source_gp <- ggplotly(source_p, tooltip="text") %>%
    layout(title = list(text = paste0('Total funding available from DWSRF and other federal and state funds',
                                      '<br>',
...
for(i in 1:length(source_gp$x$data)) {
  ...
  source_gp$x$data[[i]]$name <- gsub("Other Federal and State Funds,", "Other Federal and State Funds", source_gp$x$data[[i]]$name)
}
