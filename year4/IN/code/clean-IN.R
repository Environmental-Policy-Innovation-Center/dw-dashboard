...
#     dplyr::filter(lead_type == "both")
  ...
  ######## Decision: c("DW224189 00", "DW250443 00") --> LSLR

  in_clean <- in_clean |>
    dplyr::mutate(
      project_description = ifelse(
        project_type == "Lead" & project_id %in% c("DW224189 00", "DW250443 00"),
  ...
  in_clean <- in_clean |>
    dplyr::mutate(
      pwsid = dplyr::case_when(
        borrower == "Jackson County Water Utility" & pwsid == "IN5236003/, IN5236008" ~ "IN5236003",
        .default = pwsid
      )
    )    
####### SANITY CHECKS END #######
