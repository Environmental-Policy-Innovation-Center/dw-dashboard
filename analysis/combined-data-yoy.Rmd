...
compare_all_funds <- project_type_df |>
  dplyr::select(state, state_fiscal_year, total_project_cost, total_project_cost_ef, total_requested_amount, total_funding_amount) |>
...
# create calculated columns that will be "what's left" so that the total and the proportions are correct in plots
  dplyr::mutate(cost_less_available = total_project_cost - replace_na(total_funding_available, 0),
         ...
         total_demand = ifelse(total_requested_amount != 0, total_requested_amount, total_project_cost)) #2025/11/03, changing default order to (1)requested amount, (2) project cost based on 2025/10/30 funding tracker meeting
...
```{r}  
  ...
  compare_demand_available_funding <- compare_all_funds |>
    dplyr::select(state, state_fiscal_year, total_demand, total_funding_available, total_funding_amount, total_project_cost_ef) |>
    dplyr::mutate(
      total_funding_amount = ifelse(total_funding_amount == 0, total_project_cost_ef, total_funding_amount) #2025/11/03 per Lauren's request, if the total_funding_amount is 0, use the total project costs for projects expecting funding
    ) |>
    dplyr::select(-total_project_cost_ef) |>
...
comp_dem_avail_fund_type <- project_type_df |>
  dplyr::select(state, state_fiscal_year, project_type, total_project_cost, total_requested_amount, total_funding_amount, total_project_cost_ef) |> # 2025/11/03, use total_project_cost_ef if total_funding_amount is 0 per Lauren's request
  dplyr::group_by(state, state_fiscal_year, project_type) |>
  dplyr::summarize(
    total_project_cost = sum(total_project_cost, na.rm=TRUE),
    total_project_cost_ef = sum(total_project_cost_ef, na.rm = TRUE),
    total_requested_amount = sum(total_requested_amount, na.rm=TRUE),
    total_funding_amount = sum(total_funding_amount, na.rm=TRUE),
          .groups = "drop") |>
  dplyr::left_join(financial_by_project_type, by=c("state","state_fiscal_year", "project_type")) |>
  dplyr::mutate(
      total_funding_available = replace_na(total_funding_available, 0), 
      total_funding_amount = ifelse(total_funding_amount == 0, total_project_cost_ef, total_funding_amount), #2025/11/03 per Lauren's request, if the total_funding_amount is 0, use the total project costs for projects expecting funding
      total_demand = ifelse(total_requested_amount != 0, total_requested_amount, total_project_cost) #2025/11/03, changing default order to (1)requested amount, (2) project cost based on 2025/10/30 funding tracker meeting
      ) |> 
  dplyr::select(-total_project_cost, -total_project_cost_ef, -total_requested_amount) |>
  tidyr::pivot_longer(cols=c(total_demand, total_funding_available, total_funding_amount)) |>
  dplyr::mutate(
...
for (type in levels(comp_dem_avail_fund_type$project_type)){

filtered_data <- comp_dem_avail_fund_type |>
  ...
  comp_dem_avail_fund_type_p<- filtered_data |>
    dplyr::filter(project_type == type) |>
    ggplot2::ggplot(ggplot2::aes(x=state_fiscal_year, y=value, fill=name, text=plot_str)) + 
    ...
    labs(x="", y="", 
         ...
         subtitle=get_subtitle_str(comp_dem_avail_fund_type$state_fiscal_year, state_name)) + 
    epic_chart_theme + 
    theme(legend.position="bottom")
  ...
  comp_dem_avail_fund_type_gp <- plotly::ggplotly(comp_dem_avail_fund_type_p, tooltip="text") |>
    plotly::layout(title = list(text = paste0('Comparing Demand for Funds to Available Funds and Funds for projects on Fundable List: ', type,
                                      '<br>',
                                      get_subtitle_str(comp_dem_avail_fund_type$state_fiscal_year, state_name)))
           )
  ...
  #compare only available funds and demand
  comp_dem_avail_type_p<- filtered_data |>
    dplyr::filter(project_type == type) |>
    dplyr::filter(!name == "On Fundable List") |>
    ggplot2::ggplot(ggplot2::aes(x=state_fiscal_year, y=value, fill=name, text=plot_str)) + 
    ggplot2::geom_bar(stat="identity", position="dodge") + 
    ggplot2::scale_y_continuous(labels=label_currency()) + 
    ggplot2::scale_x_discrete(drop = FALSE) +
    ...
    labs(x="", y="", 
         title= paste0("Comparing Demand for Funds to Available Funds: ",type ), 
         subtitle=get_subtitle_str(comp_dem_avail_fund_type$state_fiscal_year, state_name)) + 
    epic_chart_theme + 
    theme(legend.position="bottom")

  comp_dem_avail_type_gp <- plotly::ggplotly(comp_dem_avail_type_p, tooltip="text") |>
    plotly::layout(title = list(text = paste0('Comparing Demand for Funds to Available Funds: ', type,
                                      '<br>',
                                      get_subtitle_str(comp_dem_avail_fund_type$state_fiscal_year, state_name)))
           )
  ...
  if (save_plots) {
    run_save_plots(
      gg_plot=comp_dem_avail_type_p,
      gp_object=comp_dem_avail_type_gp,
      name = paste0(
        gsub("_", "-", janitor::make_clean_names(type)),
...
compare_ec_funds <- project_type_df %>%
  ...
  left_join(
    ec %>% 
      select(state, state_fiscal_year, total_funding_available) |> 
      dplyr::group_by(state, state_fiscal_year) |>
      dplyr::summarise(total_funding_available = sum(total_funding_available), .groups = "drop")
      ) %>%   
# create calculated columns that will be "what's left" so that the total and the proportions are correct in plots
  mutate(cost_less_available = total_project_cost - replace_na(total_funding_available, 0),
         cost_less_available = ifelse(cost_less_available < 0, 0, cost_less_available),
