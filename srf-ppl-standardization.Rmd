...
co_clean <- co_raw %>%
  ...
  mutate(population = as.numeric(str_replace_all(pop,"[^0-9.]", "")),
         ...
         state_score = as.numeric(str_replace_all(pts,"[^0-9.]", "")),
         ) %>%
...
fl_fund <- fread(paste0(ppl_folder_path, "9-Florida_ppl_fundable.csv"),
  ...
  mutate(funding_amount = as.numeric(str_replace_all(authorized_loan_amount,"[^0-9.]","")),
         ...
         state_score = as.numeric(str_replace_all(priority_score,"[^0-9.]",""))) %>%
...
me_comp <- fread(paste0(ppl_folder_path, "19-Maine_PPL_Comprehensive.csv"),
  ...
  mutate(borrower = str_squish(water_system),
         ...
         # NOTE: typically state_score would be a numeric column, but in Maine's case, they
         # denote projects as BP and BP-EC, which is information we want to retain, so treat as text
...
nv_clean <- nv_raw %>%
  ...
  mutate(state_rank = as.numeric(str_replace_all(priority_number, "[^0-9.]", "")),
         state_score = as.numeric(str_replace_all(revised_points, "[^0-9.]", "")),
...
nc_clean <- nc_raw %>%
  # format numeric columns
  mutate(state_score = as.numeric(str_replace_all(priorit_y_points, "[^0-9.]", "")),
         population = as.numeric(str_replace_all(service_populati_on, "[^0-9.]", "")),
...
ri_comp <- fread(paste0(ppl_folder_path, "39-RhodeIsland_PPL_Comprehensive.csv"),
  ...
  mutate(
         ...
         population = as.numeric(str_replace_all(pop_served,"[^0-9.]", "")),
         state_score = as.numeric(str_replace_all(scores_total,"[^0-9.]", ""))
         ) %>%
  # process text columns
  mutate(borrower = str_squish(system_name),
