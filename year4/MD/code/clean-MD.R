clean_md_y4 <- function() {
  ...
  md_ppl_comprehensive <- data.table::fread("year4/MD/data/MD_Y4_SFY27_Comprehensive_List.csv",
    ...
    janitor::clean_names() |>
    dplyr::mutate(
      list = "SFY27 comprehensive"
    ) |>
    tidyr::drop_na(project_number) 
        
  md_ppl_fundable <- data.table::fread("year4/MD/data/MD_Y4_SFY27_Fundable_List.csv",
                  colClasses = "character", na.strings = "") |>
    janitor::clean_names() |>
    dplyr::mutate(
      list = "SFY27 fundable",
      ...
      principal_forgiveness = 
        ...
        convert_to_numeric(iija_general_pf, fill_na_0 = TRUE) +
...
####### Decision: 4 projects classified as both 
