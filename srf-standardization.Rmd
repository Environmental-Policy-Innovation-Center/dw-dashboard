...
## Combined all fundable projects into one file
# -> (29,14)
...
funded_states <- web_combined_clean %>%
...
only_applicant_states <- web_combined_clean %>%
  filter(Category == "2") %>%
  mutate(Count = 1,
         DAC = as.numeric(ifelse(`Meets State Disadvantaged Criteria` == "Yes","1","0")),
         `Principal Forgiveness` = as.numeric(`Principal Forgiveness`),
...
# Writing to temp 
write.csv(web_combined_clean, file.path(tempdir(), filename), row.names=FALSE)

#Putting in Bucket
put_object(
  file = file.path(tempdir(), "web_ppl_combined_clean.csv"),
