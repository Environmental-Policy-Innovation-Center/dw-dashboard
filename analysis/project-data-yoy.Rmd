...
if (nrow(project_type_df %>% filter(project_type == "Emerging Contaminants (EC)" & !is.na(dac_projects))) > 0) {
  
  dac_comp_ec <- project_type_df %>%
    select(state_fiscal_year, project_type, dac_projects, ndac_projects, dac_ni_projects, projects) %>% 
    filter(project_type == "Emerging Contaminants (EC)") %>%
    pivot_longer(cols=c("dac_projects", "ndac_projects", "dac_ni_projects")) %>%
    mutate(
      disadvantaged = case_when(
        name == "dac_projects" ~ "Yes",
...
```{r}
if(nrow(project_type_df %>% filter(project_type == "Emerging Contaminants (EC)" & !is.na(dac_ef_projects))) > 0) {
...
if(sum(project_type_df$projects, na.rm=TRUE) > 0) {
  ...
  pt_applicants <- project_type_df %>%
    ...
    mutate(
      project_type = factor(project_type, levels=c("Both Lead and EC","Emerging Contaminants (EC)", "Lead", "General")),
      pct_str = as.character(round(projects/projects_combined,3)*100),
...
```{r}
state_projects <- aws.s3::s3read_using(read.csv, object="s3://water-team-data/clean_data/srf_project_priority_lists/dwsrf-funding-tracker-all-projects.csv") |>
  janitor::clean_names() |>
...
sc_dac_ef <- state_projects |>
  ...
  dplyr::mutate(
    ...
    interaction = interaction(small_community, disadvantaged),
  interaction = dplyr::case_when(
    ...
    interaction == "No.No Information" ~ "Not small community, No Information",
    interaction == "Yes.No Information" ~"Small community, No Information",
  )
  ) 
...
pc_by_type_by_dac <- all_projects |>
   dplyr::filter(state == state_name) |>
   dplyr::group_by(state_fiscal_year) |>
   dplyr::filter(!all(is.na(disadvantaged))) |>
   dplyr::ungroup() |>
