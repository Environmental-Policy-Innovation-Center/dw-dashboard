...
all_grants_pf_gp <- ggplotly(all_grants_pf_p, tooltip="text") %>%
  layout(title = list(text = paste0('Principal Forgiveness by Fed Cap Grant',
                                    '<br>',
                                    subtitle_str)))
...
htmlwidgets::saveWidget(partial_bundle(base_fed_cap_grants_gp) %>%
 ...
 put_object(
   ...
   object = "/funding-tracker/TX/fed-cap-grant-test.html",
...
set_asides_gp <- ggplotly(set_asides_pct_p, tooltip="text") %>%
