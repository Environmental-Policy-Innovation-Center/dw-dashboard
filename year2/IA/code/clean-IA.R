...
clean_ia <- function() {
  ...
  ia_clean <- read.csv("./year2/IA/data/Attachment-1-Table-1.csv") %>%
    ...
    mutate(community_served = as.character(NA), 
           ...
           requested_amount = convert_to_numeric(current_funding_request), 
           funding_amount = convert_to_numeric(total_loan_amount_to_date), 
           ...
           population = convert_to_numeric(pop, FALSE),
