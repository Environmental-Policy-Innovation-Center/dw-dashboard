clean_md_y1 <- function() {
  ...
  md_ppl_comprehensive <- data.table::fread("year1/MD/data/MD_Y1_SFY24_Base_IIJA_Gen_Supp_Comprehensive_List.csv",
                  colClasses = "character", na.strings = "") |>
    janitor::clean_names() |>
    dplyr::mutate(
      list = "SFY24 comprehensive"
    )
  ...
  md_ppl_fundable <- data.table::fread("year1/MD/data/MD_Y1_SFY24_Base_IIJA_Gen_Supp_Fundable_List.csv",
                  colClasses = "character", na.strings = "") |>
    janitor::clean_names() |>
    dplyr::mutate(
      list = "SFY24 fundable",
      ...
      principal_forgiveness = convert_to_numeric(dwsrf_base_loan_forgive, fill_na_0 = TRUE) + convert_to_numeric(bil_loan_forgive, fill_na_0 = TRUE),
    ) |>
    dplyr::select(project_number, list, dwsrf_total_funding, expecting_funding, dwsrf_base_loan_forgive, bil_loan_forgive, principal_forgiveness)
