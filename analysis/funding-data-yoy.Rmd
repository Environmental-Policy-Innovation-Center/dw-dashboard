...
if (state_abbr == "NJ") {
  all_grants <- all_grants |>
    dplyr::filter(
      !(state_fiscal_year == "2023" & fed_cap_grant == "Base, IIJA Gen Supp, LSLR, and EC")
    )
}                           
...
## Funding Sources Fed Cap Grant Breakdown
```{r}
funding_source_breakdown <- financial |>
  dplyr::select(state_fiscal_year, fed_cap_grant, total_funding_available, total_funding_available_state_funds) |>
  dplyr::group_by(state_fiscal_year, fed_cap_grant) |>
  dplyr::summarise(
    total_srf_funds = sum(total_funding_available, na.rm = TRUE),
    total_state_funds = sum(total_funding_available_state_funds, na.rm = TRUE)
   ) |>
  tidyr::pivot_longer(cols = -c(state_fiscal_year, fed_cap_grant), names_to = "source") |>
  dplyr::mutate(
    source = dplyr::case_when(
      source == "total_srf_funds" ~ "DWSRF Funds",
      .default = "Other Funds Channeled Through SRFs"),
    source = forcats::as_factor(source),
    source = forcats::fct_relevel(source, "Other Funds Channeled Through SRFs","DWSRF Funds"),  
    plot_str = paste0(fed_cap_grant, " ", source, ": ", format_currency(value) )
    )
    
 other_sources <- funding_source_breakdown |>
  dplyr::ungroup() |>
  dplyr::filter(grepl("Other", source)) |>
  dplyr::group_by(state_fiscal_year) |>
  dplyr::summarise(value = sum(value)) |>
  dplyr::mutate(
    fed_cap_grant = "Aggregate",
    source = "Other Funds Channeled Through SRFs",
    plot_str = paste0("Other Funds Channeled Through SRFs: ", format_currency(value) )
  )    
    
 
source_p <- funding_source_breakdown |>
  dplyr::filter(!grepl("Other", source)) |>
  dplyr::filter(!value == 0) |>
  dplyr::bind_rows(other_sources) |>
  ggplot(aes(x=state_fiscal_year, y=value, fill=fed_cap_grant, alpha=source, text=plot_str)) + 
    geom_bar(stat="identity", position="stack") + 
    scale_alpha_manual(values = c("Other Funds Channeled Through SRFs" = 0.5, "DWSRF Funds" = 1), name = "") +
    scale_y_continuous(labels=label_currency()) + 
    labs(x="", y="", fill="", alpha="Source",
         title = "Total funding available from DWSRF and other funds channeled through SRFs",
         subtitle=get_subtitle_str(financial$state_fiscal_year, state_name)) + 
    scale_fill_manual(values=fed_cap_grant_colors) + 
    ggplot2::scale_x_discrete(drop = FALSE) +
    epic_chart_theme

source_gp <- ggplotly(source_p, tooltip="text") %>%
    layout(title = list(text = paste0('Total funding available from DWSRF and other funds channeled through SRFs',
                                      '<br>',
                                      get_subtitle_str(financial$state_fiscal_year, state_name)
                                      ))) 

# Clean up legend labels
for(i in 1:length(source_gp$x$data)) {
  source_gp$x$data[[i]]$name <- gsub("\\(|\\)", "", source_gp$x$data[[i]]$name)
  source_gp$x$data[[i]]$name <- gsub(",", ", ", source_gp$x$data[[i]]$name)
  source_gp$x$data[[i]]$name <- gsub(", Aggregate", ", ", source_gp$x$data[[i]]$name)
  source_gp$x$data[[i]]$name <- gsub("Other Funds Channeled Through SRFs,", "Other Funds Channeled Through SRFs", source_gp$x$data[[i]]$name)
}

 if (save_plots) {
    run_save_plots(gg_plot = source_p,
                   gp_object = source_gp,
                   name = "funding-sources-breakdown-yoy")
    }
...
grants <- financial |>
  ...
  dplyr::mutate(
    ...
    plt_str = paste0(fed_cap_grant, ", ", name, ": ", scales::dollar(value))
    )
...
for (category in names(grant_sets)) {
  cap_grant_subset <- grants |>
    ...
    dplyr::arrange(state_fiscal_year, fed_cap_grant, desc(name))

# Check if we have multiple segments per year (stacked case)
segments_per_year <- cap_grant_subset |>
  group_by(state_fiscal_year) |>
  summarise(n_segments = n()) |>
  pull(n_segments) |>
  max()

# Use different approach based on whether we have stacking
if (segments_per_year > 1) {
  # Multiple segments - use position_stack with group_var
  p <- ggplot2::ggplot(
      data = cap_grant_subset |> mutate(group_var = interaction(fed_cap_grant, name)),
      aes(
        x = state_fiscal_year, 
        y = value, 
        fill = fed_cap_grant)
    ...
    ggplot2::geom_col(
      aes(alpha = name, group = group_var, text = plt_str), 
      position = position_stack(), 
      width = 1) +
    ggplot2::geom_text(
      aes(
  ...
  # Single segment per year - use simple y = value/2
  p <- cap_grant_subset |>
    ggplot2::ggplot(
      aes(
        x = state_fiscal_year, 
    ...
    ggplot2::geom_col(
      aes(alpha = name, text = plt_str), 
      position = position_stack(), 
      width = 1) +
    ggplot2::geom_text(
      aes(
        label = name,
        y = value / 2),  # Simple midpoint for single bars
      color = "white", 
      show.legend = FALSE)
}
...
p <- p +
  ggplot2::labs(
    ...
    subtitle = paste(state_name, "-", category)
  ) +
...
## TN specific plot for EC category 
##TODO: revisit
# p <- cap_grant_subset |>
#     ggplot2::ggplot(
#       aes(
...
#         fill = fed_cap_grant)
#     ) +
#     ggplot2::geom_col(aes(alpha = name), width = 1) +
#     ggplot2::geom_text(
#       aes(label = name,
#         y = value/2
#       ),
...
leveraged <- financial |>
    dplyr::select(fed_cap_grant, state_fiscal_year, funds_reserved_leveraging) |>
    dplyr::mutate(state_fiscal_year = as.factor(state_fiscal_year)) |>
    dplyr::filter(!is.na(funds_reserved_leveraging) & funds_reserved_leveraging > 0) |>
    dplyr::mutate(leveraged_funds_str = format_currency(funds_reserved_leveraging))
  
  leveraged_yoy <- leveraged |>
    dplyr::group_by(state_fiscal_year) |>
    dplyr::summarize(combined_leveraged = sum(funds_reserved_leveraging))
  
  leveraged <- leveraged |>
    dplyr::left_join(leveraged_yoy, by="state_fiscal_year") |>
    dplyr::mutate(pct_str = as.character(round((funds_reserved_leveraging/combined_leveraged),3)*100),
         plot_str = paste0(fed_cap_grant, ", ", state_fiscal_year, ':<br>',
                           leveraged_funds_str, " (", pct_str, "%)"))
  
  leveraged_p <- ggplot(leveraged, aes(x=state_fiscal_year, y=funds_reserved_leveraging, 
                                       fill=fed_cap_grant, text=plot_str)) + 
    ggplot2::geom_col(width=0.5) + 
    scale_y_continuous(labels=label_currency()) + 
    ggplot2::scale_x_discrete(drop=FALSE) +
    labs(x="", y="", fill="",
         title="Funds Reserved for Leveraging",
         subtitle=get_subtitle_str(levels(leveraged$state_fiscal_year), state_name)) + 
    scale_fill_manual(values=fed_cap_grant_colors) + 
    epic_chart_theme
  
  leveraged_gp <- ggplotly(leveraged_p, tooltip="text") %>%
    layout(title = list(text = paste0('Funds Reserved for Leveraging',
                                      '<br>',
                                      get_subtitle_str(levels(leveraged$state_fiscal_year), state_name))))
...
reserved_leveraged <- financial |>
    dplyr::select(fed_cap_grant, state_fiscal_year, funds_reserved_leveraging) |>
    dplyr::mutate(state_fiscal_year = as.factor(state_fiscal_year)) |>
    dplyr::filter(!is.na(funds_reserved_leveraging) & funds_reserved_leveraging > 0) |>
    dplyr::mutate(leveraged_funds_str = format_currency(funds_reserved_leveraging))
  
  reserved_leveraged_yoy <- reserved_leveraged |>
    dplyr::group_by(state_fiscal_year) |>
    dplyr::summarize(combined_leveraged = sum(funds_reserved_leveraging))
  
  reserved_leveraged <- reserved_leveraged |>
    dplyr::left_join(reserved_leveraged_yoy, by="state_fiscal_year") |>
    dplyr::mutate(pct_str = as.character(round((funds_reserved_leveraging/combined_leveraged),3)*100),
         plot_str = paste0(fed_cap_grant, ", ", state_fiscal_year, ':<br>',
                           leveraged_funds_str, " (", pct_str, "%)"))
  
  reserved_leveraged_p <- ggplot(reserved_leveraged, aes(x=state_fiscal_year, y=funds_reserved_leveraging, 
                                       fill=fed_cap_grant, text=plot_str)) + 
    ggplot2::geom_col(width=0.5) + 
    scale_y_continuous(labels=label_currency()) + 
    ggplot2::scale_x_discrete(drop=FALSE) +
  ...
  reserved_leveraged_gp <- ggplotly(reserved_leveraged_p, tooltip="text") %>%
    layout(title = list(text = paste0('Funds Reserved for Leveraging',
                                      '<br>',
                                      get_subtitle_str(levels(reserved_leveraged$state_fiscal_year), state_name))))
  
  
  if (save_plots) {
    run_save_plots(gg_plot = reserved_leveraged_p,
                   gp_object = reserved_leveraged_gp,
                   name = "leveraged-funds-reserved-yoy")
    }

  reserved_leveraged_gp
}
...
unutilized <- financial |>
  dplyr::select(state, state_fiscal_year, fed_cap_grant, unutilized_fcg, pf_carryover, total_carryover) |>
  #to remove total when both values exist
  mutate(
    total_carryover = if_else(
      replace_na(unutilized_fcg > 0, FALSE) | replace_na(pf_carryover > 0, FALSE),
      NA,
      total_carryover
    )
  ) |>
  tidyr::pivot_longer(cols = c(unutilized_fcg, pf_carryover, total_carryover)) |>
  dplyr::filter(value > 0) |>
  dplyr::mutate(
    name = dplyr::case_when(
      name == "unutilized_fcg" ~ "General",
      name == "pf_carryover" ~ "PF",
      name == "total_carryover" ~ "Total"
    ),
    text = paste0(fed_cap_grant, ", ", state_fiscal_year, ":<br>",
                  name, ": ", format_currency(value))
  ) |>
  dplyr::mutate(
    name = forcats::fct_relevel(name, "General", "PF", "Total")  # General bottom, PF top
  ) |>
  dplyr::arrange(state_fiscal_year, fed_cap_grant, name)  # respects factor order

if (nrow(unutilized) > 0) {

  plot_title <- ifelse(state_abbr == "IN" ,  "Unutilized Funds (Carryover)", "General and Principal Forgiveness (PF) Unutilized Funds (Carryover)")
...
if (state_abbr == "OH") {
  ...
  unutilized_p <- ggplot2::ggplot(unutilized) +
    ggplot2::geom_col(
      ...
      position = ggplot2::position_stack(reverse = TRUE),  # reverses stacking order
      width = 1) +
    ggplot2::geom_text(
      data = unutilized |> dplyr::mutate(label = ifelse(name == "PF", "PF", "")), 
      ggplot2::aes(
        state_fiscal_year, 
        value, 
        label = label,  
        fill = fed_cap_grant,  # Add this
        alpha = name           # Add this
      ),
      color = "white", 
    ...
    ggplot2::geom_label(
      data = unutilized |>
        ...
        dplyr::mutate(
          cumval = cumsum(value),
          midpoint = cumval - (value / 2)
        ) |>
        dplyr::ungroup() |>
        dplyr::filter(name == "PF"),
      ggplot2::aes(
        x = state_fiscal_year,
        y = midpoint,
        label = paste0("PF: ", format_currency(value)),
        group = fed_cap_grant
      ),
      color = "black",
  ...
  # rebuild plot WITHOUT geom_text
  unutilized_p_clean <- ggplot2::ggplot(unutilized) +
    ggplot2::geom_col(
      ggplot2::aes(
        state_fiscal_year, 
        value, 
    ...
    ggplot2::scale_y_continuous(
      ...
      minor_breaks = function(lims) seq(lims[1], lims[2], by = 500000)
    ) + 
    ggplot2::scale_x_discrete(drop = FALSE) +
    ggplot2::scale_fill_manual(values = fed_cap_grant_colors) + 
    ggplot2::scale_alpha_manual(values = c("General" = 1, "PF" = 0.8), name = "") + 
  ...
  pf_labels <- unutilized |>
    dplyr::group_by(state_fiscal_year, fed_cap_grant) |>
    dplyr::mutate(cumval = cumsum(value)) |>
    dplyr::ungroup() |>
    dplyr::filter(name == "PF")

  # map factor levels to numeric positions
  year_levels <- levels(unutilized$state_fiscal_year)

  pf_labels <- pf_labels |>
    dplyr::mutate(
      x_numeric = as.numeric(state_fiscal_year)  # factor -> integer position
    )

  unutilized_gp <- plotly::ggplotly(unutilized_p_clean, tooltip = "text") |>
    ...
    list(
      x    = pf_labels$x_numeric,
      y    = pf_labels$cumval,   # label position
      text = paste0("PF: ", format_currency(pf_labels$value))
    ),
      function(x, y, text) {
        list(
          x           = x,
...
unutilized_p <- ggplot2::ggplot(unutilized) +
    ...
    ggplot2::geom_text(
      data = unutilized |> dplyr::mutate(label = dplyr::case_when(
        name == "PF" ~ name,
        name == "Total" ~ name,
        name == "General" ~ name,
        .default = ""
    ...
    ggplot2::scale_x_discrete(drop = FALSE) +
    ggplot2::scale_fill_manual(values = fed_cap_grant_colors) + 
    ggplot2::scale_alpha_manual(values = c("General" = 1, "PF" = 0.8, "Total" = 1), name = "") + 
    ggplot2::labs(x="", y="", fill="",
          title = plot_title,
          subtitle = get_subtitle_str(unutilized$state_fiscal_year, state_name)) + 
    ggplot2::guides(
...
total_carryover <- financial |>
  dplyr::select(state, state_fiscal_year, fed_cap_grant, total_carryover) |>
  dplyr::filter(total_carryover > 0) |>
  dplyr::mutate(
    name = "Total",
    text = paste0(fed_cap_grant, ", ", state_fiscal_year, ":<br>",
                  name, ": ", format_currency(total_carryover))
  ) 

total_carryover_p <- ggplot2::ggplot(total_carryover) +
    ggplot2::geom_col(
      ggplot2::aes(
        state_fiscal_year, 
        total_carryover, 
    ...
    ggplot2::scale_fill_manual(values = fed_cap_grant_colors) + 
    ggplot2::labs(x="", y="", fill="",
          title = "Total Carryover",
          subtitle = get_subtitle_str(total_carryover$state_fiscal_year, state_name)) + 
    ggplot2::guides(
      alpha = FALSE,
      label = FALSE) +
    epic_chart_theme
  ...
  total_carryover_gp <- ggplotly(total_carryover_p, tooltip="text") |>
    layout(title = list(text = paste0( "Total Carryover",
                                        '<br>',
                                        get_subtitle_str(total_carryover$state_fiscal_year, state_name)
                                        )))

    if (save_plots) {
      run_save_plots(gg_plot = total_carryover_p,
                    gp_object = total_carryover_gp, 
                    name = "total-carryover-yoy")
      }
...
base_pf_calc <- base_pf_combined %>%
  ...
  mutate(pf_category = "Remaining Base Grants",
         total_pf_amt = total_fcg - combined_pf_amt) 

base_pf_combined <- bind_rows(base_pf_combined, base_pf_calc) %>%
  select(-combined_pf_amt) %>%
  mutate(
    pf_category = factor(pf_category, levels=c("Remaining Base Grants", "Discretionary PF", "DAC PF")),
...
set_asides_fcg_group <- set_asides %>%
  group_by(fed_cap_grant, state_fiscal_year) %>%
  summarize(total_sa_amt = sum(total_sa_amt, na.rm = TRUE),
            total_fcg = max(total_fcg),
...
if(sum(set_asides_percent$total_sa_pct, na.rm=TRUE) > 0) {
  ...
  set_asides_pct_p2 <-  ggplot2::ggplot(set_asides_percent, 
  aes(x = state_fiscal_year, y = total_sa_pct, color = fed_cap_grant, text = total_sa_pct_str)) + 
...
# Loop through traces and hide legend for points
for (i in seq_along(set_asides_pct_gp2$x$data)) {
  if (!is.null(set_asides_pct_gp2$x$data[[i]]$mode) && grepl("markers", set_asides_pct_gp2$x$data[[i]]$mode)) {
    set_asides_pct_gp2$x$data[[i]]$showlegend <- FALSE
  }
}
...
if (sum(sa_pct_base$total_sa_pct, na.rm=TRUE) >0 ) {
  ...
  sa_pct_base_p <- ggplot(sa_pct_base, aes(x=state_fiscal_year, y=total_sa_pct, 
    ...
    labs(x="", y="", color="",
         title="Percent of set-asides taken, Base grant",
  ...
  sa_pct_base_gp <- ggplotly(sa_pct_base_p, tooltip="text") %>%
    layout(title = list(text = paste0('Percent of set-asides taken, Base grant',
                                      '<br>',
...
if (sum(sa_pct_gen_supp$total_sa_pct, na.rm=TRUE) >0 ) {
  ...
  sa_pct_gen_supp_p <- ggplot(sa_pct_gen_supp, aes(x=state_fiscal_year, y=total_sa_pct, 
    ...
    scale_x_discrete(drop = FALSE) +
    labs(x="", y="", color="",
         title="Percent of set-asides taken, Gen Supp grant", 
  ...
  sa_pct_gen_supp_gp <- ggplotly(sa_pct_gen_supp_p, tooltip="text") %>%
    layout(title = list(text = paste0('Percent of set-asides taken, Gen Supp grant',
                                      '<br>',
...
if (sum(sa_pct_lead$total_sa_pct, na.rm=TRUE) >0) {
  ...
  sa_pct_lead_p <- ggplot(sa_pct_lead, aes(x=state_fiscal_year, 
    ...
    labs(x="", y="", color="",
         title="Percent of set-asides taken, Lead grant",
  ...
  sa_pct_lead_gp <- ggplotly(sa_pct_lead_p, tooltip="text") %>%
    layout(title = list(text = paste0('Percent of set-asides taken, Lead grant',
                                      '<br>',
...
if (sum(sa_pct_ec$total_sa_pct, na.rm=TRUE) >0 ) {
  ...
  sa_pct_ec_p <- ggplot(sa_pct_ec, aes(x=state_fiscal_year, y=total_sa_pct, 
    ...
    labs(x="", y="", color="",
         title="Percent of set-asides taken, EC grant",
  ...
  sa_pct_ec_gp <- ggplotly(sa_pct_ec_p, tooltip="text") %>%
    layout(title = list(text = paste0('Percent of set-asides taken, EC grant',
                                      '<br>',
