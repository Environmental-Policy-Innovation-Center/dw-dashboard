...
base_distribution <- base_distribution %>%
  add_row(name="DAC PF", 
          value=pf$total_pf_amt[pf$pf_category=="DAC PF"]) %>%
  add_row(name="Discretionary PF",
          value=pf$total_pf_amt[pf$pf_category=="Discretionary PF"]) %>%
...
```{r}
if (nrow(base_distribution %>% filter(!is.na(value))) > 0  & sum(base_distribution$value, na.rm=TRUE)>0) {
...
```{r}
if (nrow(gensupp_distribution %>% filter(!is.na(value))) > 0  & sum(gensupp_distribution$value, na.rm=TRUE)>0) {
...
```{r}
if (nrow(lead_distribution %>% filter(!is.na(value))) > 0  & sum(lead_distribution$value, na.rm=TRUE)>0) {
...
```{r}
if (nrow(ec_distribution %>% filter(!is.na(value) & sum(ec_distribution$value, na.rm=TRUE)>0)) > 0) {
...
if(sum(funds_process_comp$value, na.rm=TRUE)>0) {
...
if (sfy != "2022") {
  ...
  if ( nrow(gen_costs %>% filter(!is.na(value))) == nrow(gen_costs)  & sum(gen_costs$value, na.rm=TRUE)>0)  {
...
if (sfy != "2022") {
  ...
  if ( nrow(lead_costs %>% filter(!is.na(value))) == nrow(lead_costs) & sum(lead_costs$value, na.rm=TRUE)>0)  {
...
if (sfy != "2022") {
  ...
  if ( nrow(ec_costs %>% filter(!is.na(value))) == nrow(ec_costs)  & sum(ec_costs$value, na.rm=TRUE)>0)  {
...
if ( nrow(dacs_df %>% filter(!is.na(name))) == nrow(dacs_df)  & sum(dacs_df$value, na.rm=TRUE)>0)  {
...
if ( nrow(ef_df %>% filter(!is.na(value))) == nrow(ef_df)  & sum(ef_df$value)>0)  {
...
if ( nrow(gen_dacs_df %>% filter(!is.na(name))) == nrow(gen_dacs_df)  & sum(gen_dacs_df$value, na.rm=TRUE)>0)  {
...
if ( nrow(lead_dacs_df %>% filter(!is.na(name))) == nrow(lead_dacs_df) & sum(lead_dacs_df$value, na.rm=TRUE)>0)  {
...
if ( nrow(ec_dacs_df %>% filter(!is.na(name))) == nrow(ec_dacs_df) & sum(ec_dacs_df$value, na.rm=TRUE)>0)  {
