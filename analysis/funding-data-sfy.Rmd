...
if (nrow(base_distribution %>% filter(!is.na(value))) > 0) {
  ...
  labs(title="How are the Base grants allocated?",
       subtitle=subtitle_str)

  
  base_all_fig <- base_distribution %>% plot_ly(labels = ~name, values = ~value,
                                       hovertemplate="%{label}<br>$%{value}<extra></extra>") %>%
  add_pie(hole = 0.6, direction="clockwise", sort=FALSE, marker=list(colors = distribution_colors[as.character(base_distribution$name)])) %>% 
...
gensupp_distribution <- gensupp_distribution %>%
  ...
  filter(name!="") %>%
  mutate(name = factor(name, levels=c("Remaining Gen Supp Grant, for Loans", "Local Assistance & Other Set Asides", 
                                      "State Program Management Set Asides", "Small System TA Set Asides", 
...
if (nrow(gensupp_distribution %>% filter(!is.na(value))) > 0) {
  ...
  labs(title="How are the Gen Supp grants allocated?",
       subtitle=subtitle_str)
...
lead_distribution <- lead_distribution %>%
  ...
  filter(name!="") %>%
  mutate(name = factor(name, levels=c("Remaining LSLR Grant, for Loans", "Local Assistance & Other Set Asides", 
                                      "State Program Management Set Asides", "Small System TA Set Asides", 
                                      "Admin & TA Set Asides", "Principal Forgiveness")),
         plot_str = ifelse(value > 0, paste0("$", as.character(round(value/1000000,2)), "M"), ""))
...
if (nrow(lead_distribution %>% filter(!is.na(value))) > 0) {
  ...
  labs(title="How are the LSLR grants allocated?",
       subtitle=subtitle_str)
...
ec_distribution <- ec_distribution %>%
  ...
  filter(name!="") %>%
  mutate(name = factor(name, levels=c("Remaining EC Grant, for Loans", "Local Assistance & Other Set Asides", 
                                      "State Program Management Set Asides", "Small System TA Set Asides", 
                                      "Admin & TA Set Asides",  "Principal Forgiveness")),
         plot_str = ifelse(value > 0, paste0("$", as.character(round(value/1000000,2)), "M"), ""))
...
if (nrow(ec_distribution %>% filter(!is.na(value))) > 0) {
  ...
  labs(title="How are the EC grants allocated?",
       subtitle=subtitle_str)
...
dacs_df <- data.frame(name=c("DACs, Expecting Funding", "DACs, Not Expecting Funding", "DACs, No Info"),
  ...
  mutate(value_str = ifelse(value >0, value_str, ""),
         name = factor(name, levels=c("DACs, No Info", "DACs, Not Expecting Funding", "DACs, Expecting Funding")))
...
ef_df <- data.frame(name=c("Expecting Funding, DAC", 
  ...
  mutate(value_str = ifelse(value >0, value_str, ""),
         name = factor(name, levels=c("Expecting Funding, No Info", "Expecting Funding, Not DAC", "Expecting Funding, DAC")))
