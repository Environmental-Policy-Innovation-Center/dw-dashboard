...
base_pf_pct <- base_pf_pct %>%
  ...
  mutate(name = factor(name, levels=c("Remaining Base Funds", "DAC PF", "Discretionary PF")))
```
...
lead_costs <- data.frame(category=c("Available Funds", "Unmet Demand"),
...
lead_costs$category <- factor(lead_costs$category, levels=c("Unmet Demand", "Available Funds"))
...
### EC
```{r}
ec_costs <- data.frame(category=c("Available Funds", "Unmet Demand"),
                         amount=c(57910960, (175000000-57910960)),
                         amount_str=c("$57M", "$175M"),
                         amount_pct=c("33%", "67%"))
#NOTE: Have to plot the bigger number first for it to appear intuitively
ec_costs$category <- factor(ec_costs$category, levels=c("Unmet Demand", "Available Funds"))
